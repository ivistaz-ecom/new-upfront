...
//   return (
//     <>
//       <div className="mb-10 relative container mx-auto">
//   <Slider {...settings}>
//     {data.length > 0 &&
//       data.map((post) => (
...
function PauseOnHover() {
  ...
  function NextArrow(props) {
    ...
    return (
      <div
        className="absolute -top-16 right-2 z-10 p-2 text-4xl cursor-pointer rounded-full fill-current text-gray-400"
        onClick={onClick}
      >
        <FaCircleArrowRight />
      </div>
  ...
  function PrevArrow(props) {
    ...
    return (
      <div
        className="absolute -top-16 right-12 z-10 p-2 cursor-pointer text-4xl rounded-full fill-current text-gray-400"
        onClick={onClick}
      >
        <FaCircleArrowLeft />
      </div>
  ...
  return (
    <div className="slider-container container mx-auto py-3">
      <Slider {...settings}>
        {data.length > 0 &&
          data.map((post) => (
            <div key={post.id} className="px-3">
              <Card className="bg-[#FACDD4] border-none shadow-none rounded-none">
                {post.acf && post.acf.thumbnail_image && (
                  <img
                    src={post.acf.banner_image.url}
                    alt={post.title.rendered}
                ...
                <div className='px-5 mb-5'>
                  <p className="block font-semibold text-start post-content-title"
                    dangerouslySetInnerHTML={{
                      __html: post.title.rendered,
                    }}>
                  </p>
                  <div className="h-[200px] flex flex-col justify-between text-start">
                    <div
                      dangerouslySetInnerHTML={{
                        __html: post["excerpt"]["rendered"],
                      }}
